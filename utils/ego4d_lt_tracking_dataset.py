...
class _EGO4DLTTrackingDataset(BaseDataset):
    ...
    def get_sequences(self):
        ...
        for video_uid in tqdm(vq_ann_video_uids, total=len(vq_ann_video_uids)):
            ...
            for clip in clips:
                if "exported_clip_uid" in clip and clip["exported_clip_uid"] in self.clip_info: # make sure it exists
                    response_tracks = self.get_lt_track(
                        video_uid, clip["exported_clip_uid"], frame_number_key="exported_clip_frame_number"
                    )
